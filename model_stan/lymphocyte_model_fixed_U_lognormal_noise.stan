functions {
  ...
  real label_end) {
    ...
    if(t <= label_end) {
      ...
      t_shift = t - label_end;
      L = calc_L(label_end, frac, delta, label_end, pb_w, dstar) * exp(-dstar * t_shift) + pb_w * exp(-dstar * t_shift) * calc_U(label_end, frac, delta, label_end) / (dstar - delta) * (exp((dstar - delta) * t_shift) - 1);
    }
    return L;
  }
}
...
parameters {
  ...
  real<lower = 0., upper = 5.> pb_w;
  real<lower = 0., upper = 1.> dstar;
...
transformed parameters {
  ...
  real Lhat[T];
    if(evaluate_likelihood) {
      for (t in 1:T) {
        Lhat[t] = calc_L(ts[t], frac, delta, label_end, pb_w, dstar);
      }
  } else {
      for (t in 1:T) {
        Lhat[t] = 0;
...
model {
  if(evaluate_likelihood) {
      for (t in 1:T) {
        L[t] ~ lognormal(log(Lhat[t]), sigma);
      }
  }
}
...
generated quantities {
  vector[T] log_lik;
  for (t in 1:T) {
    log_lik[t] = lognormal_lpdf(L[t] | log(Lhat[t]), sigma);
  }
}
