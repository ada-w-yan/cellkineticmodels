...
set.seed(1)
dir_name <- "/data/projects/punim0053/ayan_old/git_repos/cellkineticmodels/scripts/two_compartment_vs_kh_files/"
if(!dir.exists(dir_name)) dir.create(dir_name)
quantiles_filename <- paste0(dir_name, "bar_p_quantiles_prior_width.rds")
...
# Fit 1-6 compartment models and implicit kinetic heterogenety model to data for different prior widths
...
```{r eval = FALSE}
# fit_grid <- fit_grid %>%
#   filter(p1 == 0.72, p2 == 0.016, alpha1 == 0.1)
apply_named_args(fit_grid, 1, fit_wrapper)
```

## Fig 10 A and 10 B: single set of true parameter values
...
plot_p_all(n_compartments, prior_width = 10)
```

## Fig 10C and 10D: plotting over many true parameter values, comparing homogeneous, implicit heterogeneous and explicit two-copmartment models
...
levels(bar_p_quantiles$model) <- c("homogeneous", "implicit heterogeneous", "explicit heterogeneous")
...
bar_p_quantiles %>%
  ggplot(aes(x = prior_width, y = range, group = prior_width)) +
