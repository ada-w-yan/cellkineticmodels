...
p <- p %>%
  mutate(proliferation_error = (p_median - p_E)/p_E * 100,
         turnover_error = abs(p_median - turnover)/turnover * 100,
         k = log2(two_k_minus_one + 1))
g <- ggplot(theor_error) +
  geom_line(aes(x = influx_ratio, y = proliferation_error, color = k, group = k)) +
  scale_y_log10("Error in proliferation rate") +
  theme_bw() +
  geom_point(data = p, aes(x = influx_ratio, y = proliferation_error, color = k, group = k))
g
g <- ggplot(theor_error, aes(x = influx_ratio, y = turnover_error, color = k, group = k)) +
  geom_line() +
    theme_bw() +
  geom_point(data = p, aes(x = influx_ratio, y = turnover_error, color = k, group = k))
g
```
```{r eval = FALSE, fig.width = 3, fig.height = 3}
p <- p %>%
  mutate(proliferation_error = (production_rate_by_division - p_E)/p_E * 100,
         turnover_error = abs(production_rate_by_division - turnover)/turnover * 100,
         k = log2(two_k_minus_one + 1),
         z = influx_ratio/(two_k_minus_one + 1) * 100,
         y=1/(1/influx_ratio - 1) * 100)

# ggplot(aes(x = influx_ratio, y = y))
g <- ggplot(p, aes(x = influx_ratio, y = proliferation_error)) +
  scale_y_log10("Error in proliferation rate") +
  theme_bw() +
  xlab("Influx ratio")
if(k0) {
  ...
  g <- g + geom_point(aes(color = k)) +
    theme(legend.position = "none") +
    geom_line(aes(y = y))
}

ggsave(paste0(dir_name, "proliferation_error.pdf"), g, width = 3, height = 3)
g

g <- ggplot(p, aes(x = influx_ratio, y = turnover_error)) +
  ylab("Error in turnover rate") +
  theme_bw() +
  xlab("Influx ratio")
if(k0) {
  g <- g + geom_point()
  ...
  g <- g + geom_point(aes(color = k)) +
    theme(legend.position = "none") +
    geom_line(aes(y = z, group = k))
}

# ggsave(paste0(dir_name, "turnover_error.pdf"), g, width = 3, height = 3)
g
